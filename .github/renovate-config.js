module.exports = {
  ...
  postUpgradeTasks: {
    ...
    commands: [
      ...
      'mvn dev.logchange:logchange-maven-plugin:add --non-recursive -DbatchMode -DfileName=renovate-{{{depNameSanitized}}}-{{{newVersion}}}.yml -Dtitle="Upgraded {{{depName}}} from {{{currentVersion}}} to {{{newVersion}}}" -Dauthor.name=logchange-bot;logchange-bot;team@logchange.dev -Dtype=dependency_update -Dlink.name=notes -Dlink.url={{{url}}} || true'],
    fileFilters: ['**/*.yml'],
